...
function getCachedDbPath() {
  return path.join(getCacheDir(), 'mhb_clean.db');
}

function getCachedVersionPath() {
  return path.join(getCacheDir(), 'db-version.json');
}
...
function getBundledVersionPath() {
  return [
    path.join(process.resourcesPath || '', 'db-version.json'),
    path.join(__dirname, 'db-version.json'),
    path.join(__dirname, '..', 'data', 'db-version.json'),
...
function getLocalVersion() {
  ...
  // bundled version (first install)
  try {
    const bundled = getBundledVersionPath();
    if (!bundled) return '0.0.0';
...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    ...
    const attempt = (u) => {
      https.get(
        u,
        { headers: { 'User-Agent': 'hymn-presenter' } },
        (res) => {
...
async function getDbPath() {
  ...
  if (fs.existsSync(cached)) {
    console.log('Using cached db:', cached);
    return cached;
  }
  ...
  if (bundled) {
    console.log('Using bundled db:', bundled);
    return bundled;
  }
